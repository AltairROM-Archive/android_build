...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var ALTAIR_VERSION)
    ...
    if (adb shell getprop ro.cm.device | grep -q "$ALTAIR_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.cm.device | grep -q "$ALTAIR_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
